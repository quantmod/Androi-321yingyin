 ...
 * 
 */
public class RadiaPlayer extends BaseActivity {
	private final static String TAG = "RadiaPlayer";
...
//	private VideoInfo videoInfo = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (mIntent != null) {
			...
			if (uri != null) {
				isHttp = Utils.checkUri(RadiaPlayer.this, uri);
				String content = uri.toString().replace("?", "yangguangfu");
				if (content != null)
					netUris = content.split("yangguangfu");
			}
			if (isHttp) {
				isCheckButton = Utils.isCheckUriByM3u8(RadiaPlayer.this, uri);
				if(isCheckButton){
					mVideoName.setText(mCurrentPlayList.get(position).getTitle());
					mLoadingVideoName.setText(mCurrentPlayList.get(position).getTitle());
	...
	private void initView() {
		...
		if (currentVolume <= 0) {
			mPlayerVolume.setBackgroundDrawable(RadiaPlayer.this.getResources()
			...
			case R.id.btn_back:
				if (mCurrentPlayList != null && mCurrentPlayList.size() > 1) {
					...
					if (--position >= 0 && position < n) {
						...
						isBack = true;
						Utils.startSystemPlayer(RadiaPlayer.this, mCurrentPlayList, position);
						finish();
					} else {
						position = 0;
						if (position >= 0 && position < n) {
...
//		String eCodeUri = null;
		isBack = true;
		Intent intent = new Intent(RadiaPlayer.this, RadiaPlayer.class);
		if (mVideoView != null) {
			mVideoView.stopPlayback();
		}
		if (uri != null) {
...
//				intent.putExtra("localuri", eCodeUri);
	...
	private void startRadiaPlayer() {

//		String eCodeUri = null;
		isBack = true;
		Intent intent = new Intent(RadiaPlayer.this, SystemPlayer.class);
		if (mVideoView != null) {
			mVideoView.stopPlayback();
		}
		if (uri != null) {
...
//				intent.putExtra("localuri", eCodeUri);
	...
	private void openSetPlay() {
		try {
			dialog = new Dialog(RadiaPlayer.this, R.style.player_dialog_list);
			...
			if (enabled) {
				...
				if(mCurrentPlayList !=null && mCurrentPlayList.size()>0){
					if(position == (mCurrentPlayList.size()-1)){
						mNextButton.setBackgroundDrawable(RadiaPlayer.this
				...
				// mNextButton.setBackgroundDrawable(RadiaPlayer.this.getResources().getDrawable(R.drawable.videonextbtn_bg));
			} else {
				if(mCurrentPlayList !=null && mCurrentPlayList.size()>0){
					if(position == (mCurrentPlayList.size()-1)){
						mNextButton.setBackgroundDrawable(RadiaPlayer.this
	...
	public void setPrevEnabled(boolean enabled) {
		...
		if (mPrevButton != null) {
			...
			if (enabled) {
				...
				if(mCurrentPlayList !=null && mCurrentPlayList.size()>0){
					if(position == (mCurrentPlayList.size()-1)){
						mPrevButton.setBackgroundDrawable(RadiaPlayer.this
								.getResources().getDrawable(
										R.drawable.btn_back));
					}
				}else{
...
//			if (enabled) {
//
//				// mPrevButton.setBackgroundDrawable(RadiaPlayer.this.getResources().getDrawable(R.drawable.videoprebtn_bg));
//			} else {
...
//		
	...
	public void setPlayOrPauseEnabled(boolean enabled) {
		if (mPlayOrPause != null) {
			...
			if (enabled) {
				// mPlayOrPause.setBackgroundDrawable(RadiaPlayer.this.getResources().getDrawable(R.drawable.videonextbtn_bg));
			} else {
				if (!isPaused) {
					mPlayOrPause
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case PROGRESS_CHANGED:
				...
				if (!Utils.isCheckNetAvailable(RadiaPlayer.this) && isHttp) {
	...
	Handler mExitHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case EXIT_TEXT:
				if (isBack) {
					mLoadingText.setText(RadiaPlayer.this.getBaseContext()
							.getResources()
							.getString(R.string.exit_player_text));
					mLoadingBufferingText.setText(RadiaPlayer.this
	...
	private void ConfirmExit() {// 退出确认
		aler = new AlertDialog.Builder(RadiaPlayer.this);
		aler.setTitle("提示");
			...
			if (isOnCompletion) {
				...
				aler.setNegativeButton("进入", new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						intent.setClass(RadiaPlayer.this, MainActivity.class);
						startActivity(intent);
						overridePendingTransition(R.anim.fade, R.anim.hold);
						alertDialog.hide();
						finish();
