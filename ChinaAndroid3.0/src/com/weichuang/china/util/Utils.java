...
public class Utils {
	 ...
	 * 标题背景设置
	 * */
	public static void setTitleBarChangeBackground( Context context ,FrameLayout titleBar ){ 
	 ...
	 * 界面背景设置
	 * */
	public static void setChangeBackground(Context context ,RelativeLayout background_id){
		UserPreference.ensureIntializePreference(context);  
	    int defaultColor = UserPreference.read("defaultColor", 0); 
	 ...
	 * 按钮背景设置
	 * */
	public static void setChangeButtonBackground(Context context,View view[] ){
		int caidan = UserPreference.read("caidan", 0);//菜单按钮颜色设置
		String xingzhuang = null;
		xingzhuang = UserPreference.read("xiangzhuang", null);//表示菜单按钮形状设置
		if(xingzhuang == null ){
			xingzhuang = "正方形";
		}
		if (caidan == 0) {//菜单按钮颜色设置
			Utils.color[0]  = context.getResources().getColor(R.color.lans);
			Utils.color[1]  = context.getResources().getColor(R.color.lans);
//			setButtonStyle();
			
			 if("正方形".equals(xingzhuang)){ 
					 for(int i = 0;i < view.length;i++){
						 view[i].setBackgroundDrawable(Utils.setBackgroundType("正方形")); ; 	
					 } 
		   		}else if("半圆形".equals(xingzhuang)){
			   		 for(int i = 0;i < view.length;i++){
						 view[i].setBackgroundDrawable(Utils.setBackgroundType("半圆形"));	
					 }   
		   		}else if("圆形".equals(xingzhuang)){ 
			   		 for(int i = 0;i < view.length;i++){
						 view[i].setBackgroundDrawable(Utils.setBackgroundType("圆形"));	
					 }   
		   		}
		} else {
			...
			Utils.color[1] = caidan; 
			 for(int i = 0;i < view.length;i++){
				 view[i].setBackgroundDrawable(Utils.setBackgroundType(xingzhuang));
			 }    
		}
	}
